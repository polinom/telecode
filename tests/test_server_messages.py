...
def test_handle_callback_query_uses_cached_option(monkeypatch, tmp_path):
    _set_cwd(tmp_path, monkeypatch)
    os.environ["TELECODE_ALLOWED_USERS"] = ""
    captured = {}
    ...
    monkeypatch.setattr(server, "_handle_prompt", fake_handle_prompt)
    monkeypatch.setattr(server, "telegram_answer_callback_query", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_send_message", lambda *args, **kwargs: 1)

    server._store_option_cache(888, 42, ["First", "Second"])

    callback = {
        "id": "cb1",
        "data": "opt:2",
        "from": {"id": 888, "username": "tester"},
        "message": {"message_id": 42, "chat": {"id": 888}},
    }
    server.handle_callback_query(callback, None, _dummy_telegram(), ".telecode", "claude")

    assert captured["prompt"] == "Second"


def test_inline_keyboard_callback_data_is_indexed():
    options = ["Option A", "Option B"]
    markup = server._build_inline_keyboard(options)
    assert markup["inline_keyboard"][0][0]["callback_data"] == "opt:1"
    assert markup["inline_keyboard"][1][0]["callback_data"] == "opt:2"


def test_extract_options_from_numbered_list():
    text = "Choose one:\n1. Alpha\n2) Beta\n"
    response_text, options = server._extract_options(text)
    assert response_text == "Choose one:"
